...
(function(root, factory) {
   ...
   * and resolution, mouse and keyboard interactions, and a virtual p5Camera.
   *
  ...
  p5.prototype.registerMethod("init", function p5PlayInit() {
    /**
     * The sketch p5Camera automatically created at the beginning of a sketch.
     * A p5Camera facilitates scrolling and zooming for scenes extending beyond
     * the canvas. A p5Camera has a position, a zoom factor, and the mouse
     * coordinates relative to the view.
     *
     * In p5.js terms the p5Camera wraps the whole drawing cycle in a
     ...
     * @type {p5Camera}
     */
    this.p5Camera = new Camera(this, 0, 0, 1);
    this.p5Camera.init = false;
  });
   ...
   *
   * @method loadp5Animation
   * @param {Sprite} sprite Sprite to be displayed
   */
  p5.prototype.loadp5Animation = function() {
    return construct(this.p5Animation, arguments);
  };
   ...
   *
   * @method animation
   * @param {p5Animation} anim p5Animation to be displayed
...
*/
  ...
  */
   ...
   */
     ...
     * the p5Camera is active.
     * Used in Sprite.update() to know whether to use p5Camera mouse coordinates.
     ...
     */
    this._syncp5AnimationSizes = function() {
      ...
      if (
        ...
        animations[currentp5Animation].getWidth() !== 1 &&
        animations[currentp5Animation].getHeight() !== 1
      ) {
        this.collider = this.getBoundingBox();
        this.colliderType = "image";
        this._internalWidth =
          animations[currentp5Animation].getWidth() * abs(this._getScaleX());
        this._internalHeight =
          animations[currentp5Animation].getHeight() * abs(this._getScaleY());
        //quadTree.insert(this);
      }

      //update size and collider
      if (
        animations[currentp5Animation].frameChanged ||
    ...
    this.update = function() {
      if (!this.removed) {
        ...
        if (animations[currentp5Animation]) {
          //update it
          animations[currentp5Animation].update();

          this._syncp5AnimationSizes();
        }
    ...
    this.mouseUpdate = function() {
      ...
      if (this._drawnWithCamera)
        mousePosition = createVector(p5Camera.mouseX, p5Camera.mouseY);
    ...
    this.display = function() {
      if (this.visible && !this.removed) {
        ...
        this._drawnWithCamera = p5Camera.active;
     ...
     */
    this.draw = function() {
      if (currentp5Animation !== "" && animations) {
        if (animations[currentp5Animation])
          animations[currentp5Animation].draw(0, 0, 0);
      } else {
        noStroke();
     ...
     * p5Animations require a identifying label (string) to change them.
     * The image is stored in the sprite but not necessarily displayed
     * until Sprite.changep5Animation(label) is called
     *
     * Usages:
    ...
    this.addImage = function() {
      if (typeof arguments[0] === "string" && arguments[1] instanceof p5.Image)
        this.addp5Animation(arguments[0], arguments[1]);
      else if (arguments[0] instanceof p5.Image)
        this.addp5Animation("normal", arguments[0]);
      else
     ...
     * using loadp5Animation.
     * p5Animations require a identifying label (string) to change them.
     * p5Animations are stored in the sprite but not necessarily displayed
     * until Sprite.changep5Animation(label) is called.
     *
     * Usage:
     * - sprite.addp5Animation(label, animation);
     *
     * Alternative usages. See p5Animation for more information on file sequences:
     * - sprite.addp5Animation(label, firstFrame, lastFrame);
     * - sprite.addp5Animation(label, frame1, frame2, frame3...);
     *
     * @method addp5Animation
     * @param {String} label p5Animation identifier
     * @param {p5Animation} animation The preloaded animation
     */
    this.addp5Animation = function(label) {
      var anim;

      if (typeof label !== "string") {
        print(
          "Sprite.addp5Animation error: the first argument must be a label (String)"
        );
        return -1;
      } else if (arguments.length < 2) {
        print(
          "addp5Animation error: you must specify a label and n frame images"
        );
        return -1;
      } else if (arguments[1] instanceof p5Animation) {
        var sourcep5Animation = arguments[1];

        var newp5Animation = sourcep5Animation.clone();

        animations[label] = newp5Animation;

        if (currentp5Animation === "") {
          currentp5Animation = label;
          this.animation = newp5Animation;
        }

        newp5Animation.isSpritep5Animation = true;

        this._internalWidth =
          newp5Animation.getWidth() * abs(this._getScaleX());
        this._internalHeight =
          newp5Animation.getHeight() * abs(this._getScaleY());

        return newp5Animation;
      } else {
        var animFrames = [];
        for (var i = 1; i < arguments.length; i++)
          animFrames.push(arguments[i]);

        anim = construct(pInst.p5Animation, animFrames);
     ...
     * Equivalent to changep5Animation
     *
     * @method changeImage
     * @param {String} label Image/p5Animation identifier
     */
    this.changeImage = function(label) {
      this.changep5Animation(label);
    };

    /**
     * Returns the label of the current animation
     *
     * @method getp5AnimationLabel
     * @return {String} label Image/p5Animation identifier
     */
    this.getp5AnimationLabel = function() {
      return currentp5Animation;
    };

    /**
     * Changes the displayed animation.
     * See p5Animation for more control over the sequence.
     *
     * @method changep5Animation
     * @param {String} label p5Animation identifier
     */
    this.changep5Animation = function(label) {
      if (!animations[label])
        print("changep5Animation error: no animation labeled " + label);
      else {
        currentp5Animation = label;
    ...
    this.AABBops = function(type, target, callback) {
      ...
      for (var i = 0; i < others.length; i++)
        if (this !== others[i] && !this.removed) {
          ...
          /*
        if(this.colliderType=="default" && animations[currentp5Animation]!=null)
        {
          print("busted");
          return false;
        }*/
   ...
   * the canvas. A p5Camera has a position, a zoom factor, and the mouse
   * coordinates relative to the view.
   * The p5Camera is automatically created on the first draw cycle.
   *
   * In p5.js terms the p5Camera wraps the whole drawing cycle in a
  ...
  function Camera(pInst, x, y, zoom) {
    ...
    /**
     * MouseX translated to the p5Camera view.
     ...
     * position if the p5Camera moved or zoomed.
     *
     * @property mouseX
     * @type {Number}
     */
    ...
    /**
     * MouseY translated to the p5Camera view.
     * Offsetting and scaling the canvas will not change the sprites' position
     * nor the mouseX and mouseY variables. Use this property to read the mouse
     * position if the p5Camera moved or zoomed.
     *
    ...
    /**
     * True if the p5Camera is active.
     * Read only property. Use the methods Camera.on() and Camera.off()
     * to enable or disable the p5Camera.
     *
     * @property active
     * @type {Boolean}
     */
    ...
    /**
     * Activates the p5Camera.
     * The canvas will be drawn according to the p5Camera position and scale until
    ...
    this.on = function() {
      if (!this.active) {
        p5CameraPush.call(pInst);
        this.active = true;
      }
    };

    /**
     * Deactivates the p5Camera.
     * The canvas will be drawn normally, ignoring the p5Camera's position
  ...
  function p5CameraPush() {
    var pInst = this;
    var p5Camera = pInst.p5Camera;

    //awkward but necessary in order to have the p5Camera at the center
    //of the canvas by default
    if (
      !p5Camera.init &&
      p5Camera.position.x === 0 &&
      p5Camera.position.y === 0
    ) {
      p5Camera.position.x = pInst.width / 2;
      p5Camera.position.y = pInst.height / 2;
      p5Camera.init = true;
    }

    p5Camera.mouseX = pInst.mouseX + p5Camera.position.x - pInst.width / 2;
    p5Camera.mouseY = pInst.mouseY + p5Camera.position.y - pInst.height / 2;

    if (!p5Camera.active) {
      p5Camera.active = true;
      pInst.push();
      pInst.scale(p5Camera.zoom);
      pInst.translate(
        -p5Camera.position.x + pInst.width / 2 / p5Camera.zoom,
        -p5Camera.position.y + pInst.height / 2 / p5Camera.zoom
      );
    }
  }

  //called postdraw by default
  function p5CameraPop() {
    var pInst = this;

    if (pInst.p5Camera.active) {
      pInst.pop();
      pInst.p5Camera.active = false;
    }
  }
  ...
  /**
   * An p5Animation object contains a series of images (p5.Image) that
   ...
   *
   * A sprite can have multiple labeled animations, see Sprite.addp5Animation
   * and Sprite.changep5Animation, however an animation can be used independently.
   *
   ...
   *
   * @example
   *     var sequencep5Animation;
   *     var glitch;
   *
   *     function preload() {
   *       sequencep5Animation = loadp5Animation("data/walking0001.png", "data/walking0005.png");
   *       glitch = loadp5Animation("data/dog.png", "data/horse.png", "data/cat.png", "data/snake.png");
   *     }
   *
   *     function setup() {
   ...
   *     function draw() {
   *       background(0);
   *       animation(sequencep5Animation, 100, 100);
  ...
  function p5Animation(pInst) {
    ...
    if (
      ...
      if (ext1 !== ".png") {
        pInst.print(
          "p5Animation error: you need to use .png files (filename " +
            from +
            ")"
        );
        from = -1;
      ...
      if (ext2 !== ".png") {
        pInst.print(
          "p5Animation error: you need to use .png files (filename " + to + ")"
        );
        to = -1;
      }
    ...
    // Sprite sheet mode
      ...
      //print("p5Animation arbitrary mode");
     ...
     *
     * @method clone
     * @return {p5Animation} A clone of the current animation
     */
    this.clone = function() {
      var myClone = new p5Animation(pInst); //empty
      myClone.images = [];
    ...
    this.draw = function(x, y, r) {
      ...
      if (this.visible) {
        ...
        if (!this.isSpritep5Animation) this.update();
    ...
    this.getWidth = function() {
      if (this.images[frame] instanceof p5.Image) {
        ...
        // Special case: p5Animation-from-spritesheet treats its images array differently.
  ...
  defineLazyP5Property("p5Animation", boundConstructorFactory(p5Animation));

  /**
   * Represents a sprite sheet and all it's frames.  To be used with p5Animation,
  ...
  //p5Camera push and pop
  p5.prototype.registerMethod("pre", p5CameraPush);
  p5.prototype.registerMethod("post", p5CameraPop);
